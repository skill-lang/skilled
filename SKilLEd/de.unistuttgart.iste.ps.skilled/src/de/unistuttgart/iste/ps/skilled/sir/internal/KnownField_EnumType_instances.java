...
final class KnownField_EnumType_instances extends FieldDeclaration<java.util.ArrayList<de.unistuttgart.iste.ps.skilled.sir.Identifier>, de.unistuttgart.iste.ps.skilled.sir.EnumType> implements
    ...
    public long offset() {
        ...
        for (; i < high; i++) {
            final java.util.ArrayList<? extends de.unistuttgart.iste.ps.skilled.sir.Identifier> v = (java.util.ArrayList<? extends de.unistuttgart.iste.ps.skilled.sir.Identifier>)((de.unistuttgart.iste.ps.skilled.sir.EnumType)data[i]).getInstances();
            if(null==v || v.isEmpty())
                result++;
            else {
                result += V64.singleV64Offset(v.size());
                result += baseType.calculateOffset((java.util.ArrayList<de.unistuttgart.iste.ps.skilled.sir.Identifier>)v);
            }
        }
        return result;
    }
