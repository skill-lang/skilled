		...
		for (Field f : checkedTypeDeclaration.fields) {
			...
			if ((f.fieldcontent instanceof View)) {
				...
				if (viewToCheck.fieldtype instanceof DeclarationReference) {
					var DeclarationReference viewFieldtypeDeclarationReference = viewToCheck.
					...
					if ((viewFieldtypeDeclarationReference.type instanceof Usertype) ||
						...
						isUsertype = true
						// view A.x as B c;:
						var String supertypeTypename = (viewToCheck.fieldcontent.fieldcontent.eContainer.
						...
						if (supertypeDec == null) {
							...
							if (supertypeVar == null) {
								...
								error("Error: " + supertypeVarname + " is not a Variable in " + supertypeTypename + ".",
									viewToCheck, SKilLPackage.Literals.VIEW.getEStructuralFeature(2), VIEW_ERROR)
								...
								if (!supertypeVarIsUsertype) {
									error("Error: " + supertypeVarname + " is not a Usertype or Interface variable.",
										viewToCheck, SKilLPackage.Literals.VIEW.getEStructuralFeature(2), VIEW_ERROR)
								} else if (usertype.name != usertypeSupertypeVar.name) {
									typesSearched.clear
									if (searchSupertype(usertypeSupertypeVar.name, usertype) == null) {
										error(
											"Error: " + usertypeSupertypeVar.name + " is not a supertype of " +
												usertype.name + ".", viewToCheck,
											SKilLPackage.Literals.VIEW.getEStructuralFeature(1), VIEW_ERROR)
									}
								}
							}
						}
				...
				if (!isUsertype) {
					...
					error("Error: not a usertype variable.", viewToCheck,
						SKilLPackage.Literals.VIEW.getEStructuralFeature(1), VIEW_ERROR)
				}
