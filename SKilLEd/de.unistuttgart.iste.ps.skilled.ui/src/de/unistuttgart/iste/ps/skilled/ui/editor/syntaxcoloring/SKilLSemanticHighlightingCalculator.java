...
/**
 * Custom Highlighting Calculator for SKilL. It is needed for custom syntax
 * coloring. Semantic Highlighting
 * 
 * @author Marco Link
 */
public class SKilLSemanticHighlightingCalculator implements ISemanticHighlightingCalculator {
	...
	/**
	 * Checks the type of the nodes in the syntax tree and hands over the
	 * position of the individually node with its text style.
	 * 
	 * @param resource
	 *            the resource that will be highlighted. May be
	 *            <code>null</code> in some rare cases.
	 * @param acceptor
	 *            used to announce the mapping from text-range to the style's
	 *            id. The acceptor will never be <code>null</code>.
	 */
	@Override
			...
			if ((grammarElement instanceof CrossReference)) {
				...
				// Checks for fieldcontent cross references in views.
					...
					if (fr.getFieldcontent() instanceof Data) {
						...
						acceptor.addPosition(node.getOffset(), node.getLength(),
								SKilLHighlightingConfiguration.VIEW_ID);
