...
/**
 * This class creates the Dialog window where the user can select the .skill
 * files to be combined and the name and the save location of the combined
 * document.
 * 
 * @author Leslie Tso
 *
		...
		OK.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				for (int i = 0; i < numberSelected; i++) {
					...
					if (fCombinedSave.getText().equals(fCombines[i])) {
						JOptionPane.showMessageDialog(null,
								"Cannot set one of the pre-combined files as the combined file.", "Overwrite Error",
								JOptionPane.ERROR_MESSAGE);
						return;
					}
				}
				...
				// Error message if save directory does not exist
					...
					JOptionPane.showMessageDialog(null, "Location of combined file does not exist!",
							"Invalid Save Input", JOptionPane.ERROR_MESSAGE);
				}
				// Error if user tries to save outside the workspace
				else if (!fCombinedLocation.contains(workspaceDirectory.getAbsolutePath())) {
					JOptionPane.showMessageDialog(null, "Save location is not in the workspace!",
							"Invalid Save Location", JOptionPane.ERROR_MESSAGE);
				}
