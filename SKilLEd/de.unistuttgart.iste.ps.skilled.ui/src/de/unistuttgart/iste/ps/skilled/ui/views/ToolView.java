...
public class ToolView extends ViewPart {
    ...
    private void writeToolBinaryFile() {
        MainClass.main(new String[] { "-e", path.substring(0, path.lastIndexOf('.') - 1), activeTool.getName() + ":0;" });
                ...
                if (event.detail == SWT.CHECK) {
                    ...
                    if (null != type) {
                        ...
                        if (((TreeItem) event.item).getChecked()) {
                            ...
                            Type tooltype = new Type(type.getSkillID(), type.getComment(), type.getExtends(),
                                    type.getFields(), type.getFile(), type.getName(), type.getRestrictions(),
                                    type.getTypeHints());
                    ...
                    if (hint != null) {
                        ...
                        if (((TreeItem) event.item).getChecked()) {
                            ...
                            ((Type) toolhint.getParent()).getTypeHints().add(toolhint);
                            buildTypeTree(activeTool);
                            writeToolBinaryFile();
                        } else {
                            System.out.println("hint removed");
                            for (Hint h : ((Type) hint.getParent()).getTypeHints()) {
                                if (h.getName().equals(hint.getName())) {
                                    ((TreeItem) event.item).setData(h);
                                }
                            }
                            ((Type) hint.getParent()).getTypeHints().remove(hint);
                            buildTypeTree(activeTool);
                            writeToolBinaryFile();
                        }
                    }
