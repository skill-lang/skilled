...
public class ToolView extends ViewPart {
	...
	private String path = SKilLActivator.getInstance().getBundle().getLocation().substring(16) + "tools.sf";
	...
	public void createPartControl(Composite parent) {
		String platformString = SKilLActivator.getInstance().getBundle().getLocation();
		IFile myFile = ResourcesPlugin.getWorkspace().getRoot().getFile(new Path(platformString));
		IProject project = myFile.getProject();
		IFile file = project.getFile(File.separator + ".skills");
		path = myFile.getFullPath().toOSString().substring(7) + file.getFullPath().toOSString().substring(6);
