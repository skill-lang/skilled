...
public class ToolView extends ViewPart {
					...
					if (hint != null) {
						...
						if (((TreeItem) event.item).getChecked()) {
							...
							ToolUtil.addTypeHint(activeTool.getName(), activeProject,
									getActualName(((Type) hint.getParent()).getName()), hint.getName());
							buildTypeTree(activeTool);
						} else {
					...
					if (null != field) {
						...
						if (((TreeItem) event.item).getChecked()) {
							...
							ToolUtil.addField(activeTool.getName(), activeProject,
									getActualName(field.getType().getName()), getActualName(field.getName()));
							refresh();
					...
					if (hint != null) {
						...
						if (((TreeItem) event.item).getChecked()) {
							...
							ToolUtil.addFieldHint(activeTool.getName(), activeProject,
									getActualName(((Field) hint.getParent()).getType().getName()),
									getActualName(((Field) hint.getParent()).getName()), hint.getName());
							refresh();
						} else {
	 ...
	 * @throws IOException
	 */
	private void deleteDirectoryRecursivly(File directoryToDelete) throws IOException {
