...
public class EditorCorrectInputTest {
	...
	public void testSetDefaults() {
		...
		try {
			SkillFile sk = SkillFile.open(skillFilePath, de.ust.skill.common.java.api.SkillFile.Mode.Read);
			assertTrue("no defaults set.", sk.Tools().stream()
	...
	/**
	 * Tests if you can add a type which extends another type and that type is
	 * automatically added.
	 */
	@Test
	...
	public void testAddingEnum() {
		...
		try {
			...
			assertTrue("does not contain all enum values",
					sk.Tools().stream().anyMatch(t -> t.getName().equals("testTool")
	...
	public void testExtractingTestToolNoCleanup() {
		try {
			...
			Files.copy(Paths.get("testFiles" + File.separator + "CorrectInput" + File.separator
					+ "NoCoExtractingTestToolNoCleanup.sf"), Paths.get(skillFilePath));
		} catch (IOException e) {
			fail(e.getMessage());
		}
		String builder = "lib" + File.separator + "skill_2.11-0.3.jar";
	...
	public void testAddingTypeHint() {
		...
		try {
			...
			assertTrue("PowerStrip does not have hint",
	...
	public void testViewIndexing() {
		...
		try {
			...
			assertTrue("view not correctly added to tool",
					sf.Tools().stream().anyMatch(t -> t.getName().equals("testTool") && t.getTypes().stream()
							.anyMatch(ty -> ty.getName().equals("ViewType") && ty.getFields().stream().anyMatch(
