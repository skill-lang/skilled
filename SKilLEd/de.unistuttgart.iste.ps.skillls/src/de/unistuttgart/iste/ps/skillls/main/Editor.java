...
public class Editor {
    ...
    private void removeTypeHint(Tool tool, String[] subCommands, int globalIndex) {
        ...
        Type type = findType(tool.getTypes(), typeName);
        if (type == null) {
            return;
        }
    ...
    private void addTypeHint(Tool tool, String[] subCommands, int globalIndex) {
        ...
        Type toolType = findType(tool.getTypes(), typeName);
        if (toolType == null) {
        ...
        for (Type origType : skillFile.Types().stream().filter(ty -> ty.getOrig() == null).collect(Collectors.toList())) {
            ...
            if (typeName.equals(origType.getName())
                ...
                if (tool.getTypes().stream().noneMatch(ty -> ty != null && ty.getName().equals(typeName))) {
                    ...
                    type = skillFile.Types().make(origType.getComment(), (ArrayList<String>) origType.getExtends().clone(),
                            new ArrayList<>(), origType.getFile(), origType.getName(), origType, new ArrayList<>(),
                            new ArrayList<>());
                }
                // type is enum
    ...
    private void addGroundType(Type type, Tool tool) {
        ...
        for (Type candidate : candidates) {
            ...
            if (candidateName.startsWith("typedef " + groundTypeName) || candidateName.startsWith("enum " + groundTypeName)
                ...
                // noinspection unchecked
                tool.getTypes()
                        .add(skillFile.Types().make(candidate.getComment(),
     ...
     */
    private static Type findType(Collection<Type> types, String typeName) {
        for (Type t : types) {
