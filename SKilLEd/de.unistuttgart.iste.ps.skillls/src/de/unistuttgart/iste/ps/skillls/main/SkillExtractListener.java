...
/**
 * Class for extracting a skill specification of a tool from a general skill
 * specification.
 *
...
public class SkillExtractListener extends SKilLParserBaseListener {
	 ...
	 * @throws IOException
	 *             thrown if a exception occurred while writing a type to the
	 *             outfile.
	 */
	...
	private void processTypedef(SKilLParser.TypedefContext ctx) throws IOException {
		...
		for (Type t : tool.getTypes()) {
			if (t != null
					&& t.getName().toLowerCase().indexOf(ctx.name.getText().toLowerCase()) == "typedef ".length()) {
				type = t;
				break;
			}
		}
		...
		builder.append(";\n\n");
		outFile.write(
				String.format(builder.toString(), hintsRestrictions.length() == 0 ? "" : hintsRestrictions.toString())
						.getBytes());
	}
	 ...
	 * 
	 * @param type
	 *            the type whose fields should be added
	 * @param builder
	 *            builder containing the string
	 */
	...
	private void processUsertype(SKilLParser.UsertypeContext ctx) throws IOException {
		Type type = null;
		for (Type t : tool.getTypes()) {
			if (t != null && t.getName().toLowerCase().equals(ctx.name.getText().toLowerCase())
					&& t.getFile().getPath().equals(inFile.getPath())) {
				type = t;
				break;
			}
		}
	 ...
	 * 
	 * @param type
	 *            the type whose hints should be added
	 * @param builder
	 *            builder containing the string
	 */
	 ...
	 * 
	 * @param type
	 *            the type whose restrictions should be added
	 * @param builder
	 *            builder containing the string
	 */
