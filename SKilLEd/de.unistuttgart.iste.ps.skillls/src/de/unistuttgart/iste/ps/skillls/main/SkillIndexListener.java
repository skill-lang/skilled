...
class SkillIndexListener extends SKilLParserBaseListener {
    ...
    private void reindex(SKilLParser.InterfacetypeContext interfacetype) {
        ...
        newType = skillFile.Types().make(comment, extend, fields, file, "interface " + interfacetype.name.getText(), null,
                restrictions, hints);
    ...
    private void reindex(SKilLParser.TypedefContext typedef) {
        ...
        newType = skillFile.Types().make(comment, extend, fields, file, "typedef " + typedef.name.getText() + " %s", null,
                restrictions, hints);
    ...
    private void reindex(SKilLParser.UsertypeContext usertypeContext) {
        ...
        newType = skillFile.Types().make(comment, extend, fields, file, usertypeContext.name.getText(), null, restrictions,
                hints);
        for (Field field : fields) {
            field.setType(newType);
        }
    ...
    private Field indexFieldConstant(SKilLParser.FieldContext fieldContext) {
        ...
        String name = "const " + fieldContext.constant().type().getText() + ' ' + fieldContext.constant().extendedIdentifer()
