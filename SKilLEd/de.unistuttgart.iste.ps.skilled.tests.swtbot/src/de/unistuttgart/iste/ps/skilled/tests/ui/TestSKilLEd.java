...
public class TestSKilLEd {
    ...
    public void testCreateSkillProject() {
        ...
        String testProjectFullname = null;
        try {
            for (SWTBotTreeItem ti : bot.tree().getAllItems()) {
                if (ti.getText().startsWith(testProject)) {
                    testProjectFullname = ti.getText();
                }
            }
            if (testProjectFullname != null) {
                bot.tree().getTreeItem(testProjectFullname).expand();
                bot.tree().getTreeItem(testProjectFullname).getNode("TestFile.skill");
            } else {
        ...
        bot.tree().getTreeItem(testProjectFullname).getNode(testFile).select();
    ...
    public void testOrganizeImportsQuickfix() {
        ...
        if (!(editor.getText().contains("include \"" + testFile + "\"")
                && (editor.getText().contains("include \"" + testFile3 + "\"")))) {
            throw new AssertionError();
        }
        editor.saveAndClose();
    }
    ...
    public void testImportBinary() {
        ...
        try {
            ...
            bot.textWithLabel("Select binary file to import:")
                    .setText("resources" + File.separator + "testImport" + File.separator + "age.sf");
    ...
    /**
     * 
     * This test opens a SKilL-File specified in the specification (2.3) and checks if it is opened in the specified time.
    ...
    public void openFileTimeSpecified() {
        ...
        bot.tree().getTreeItem(testProjectFullname).getNode(testFile).doubleClick();
    ...
    private boolean combineFiles(String fileName1, String fileName2, String combinedFile) {
        try {
            ...
            bot.textWithLabel("Save combined file as:")
                    .setText(workspacePath + File.separator + testProject + File.separator + combinedFile);
            bot.button("OK").click();
     ...
     */
    private void createSKilLFile(final String fileName, final String projectName) {
